...
class GomBiPage extends StatefulWidget {
  const GomBiPage({super.key, this.repository = const SqliteNoteRepository()});

  final NoteRepository repository;

  @override
  State<GomBiPage> createState() => GomBiPageState();
}

class GomBiPageState extends State<GomBiPage>
    with CategoryNotesState<GomBiPage> {
  @override
  NoteRepository get noteRepository => widget.repository;

  @override
  String get noteCategory => 'gombi';

  List<ExpansionTileController> get _controllers => expansionControllers;
  List<Note> get _userNotes => filteredCategoryNotes;
  ...
  Future<void> loadUserNotes() => loadCategoryNotes();
  ...
  Future<void> resetToDefaults() async {
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Bạn có chắc chắn muốn xóa tất cả thay đổi và khôi phục các bài viết mẫu ban đầu không?',
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                await resetCategoryNotes(SystemDefaultNotes.getGomBiNotes());
                if (!mounted) return;
                messenger.showSnackBar(
                  const SnackBar(
                    content: Text('Đã khôi phục các bài viết Gom bi mặc định!'),
                  ),
                );
              } catch (_) {
                if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: DataStateView(
        ...
        onRetry: loadUserNotes,
        child: ListView(
          children: [
            NoteSearchField(onChanged: updateNotesQuery),
            ..._userNotes.map(_buildListArticleCard),
          ],
        ),
      ),
    );
  ...
  Widget _buildListArticleCard(Note note) {
  ...
  Widget _buildGridArticleCard(Note note) {
    NoteBlock? diagramBlock;
    for (var b in note.blocks) {
    ...
    if (diagramBlock != null) {
      ...
      try {
        data = jsonDecode(diagramBlock.content);
      } catch (e) {
        data = {};
      }
      List<Ball> balls = [];
      ...
      previewWidget = Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [note.color.withOpacity(0.4), note.color.withOpacity(0.1)],
        ...
        child: Center(
          child: Icon(Icons.api_rounded, size: 48, color: note.color),
        ),
      );
    }
  ...
  Widget _buildUserArticle({required int index, required Note note}) {
    ...
    return Container(
      decoration: BoxDecoration(
        border: Border(
          bottom: BorderSide(color: Colors.blueGrey.withOpacity(0.5)),
        ),
      ),
      child: ExpansionTile(
        key: ValueKey('gombi_${note.id ?? note.title}'),
        tilePadding: const EdgeInsets.only(left: 12.0, right: 4.0),
        controller: _controllers[index],
        onExpansionChanged: (val) => _onExpansionChanged(index, val),
        leading: Container(
          width: 42,
          height: 42,
          decoration: BoxDecoration(
          ...
          child: Icon(Icons.group_work_rounded, color: note.color, size: 22),
        ),
        title: Text(
          note.title,
