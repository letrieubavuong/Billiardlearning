...
class CoBanPage extends StatefulWidget {
  const CoBanPage({super.key, this.repository = const SqliteNoteRepository()});

  final NoteRepository repository;

  @override
  State<CoBanPage> createState() => CoBanPageState();
}

class CoBanPageState extends State<CoBanPage>
    with CategoryNotesState<CoBanPage> {
  @override
  NoteRepository get noteRepository => widget.repository;

  @override
  String get noteCategory => 'coban';
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                await resetCategoryNotes(SystemDefaultNotes.getCoBanNotes());
                if (!mounted) return;
                messenger.showSnackBar(
                  const SnackBar(
                    content: Text('Đã khôi phục các bài viết Cơ bản mặc định!'),
                  ),
                );
              } catch (_) {
                if (!mounted) return;
  ...
  Widget _buildUserArticle({required int index, required Note note}) {
    ...
    return Container(
      ...
      child: ExpansionTile(
        key: ValueKey('coban_${note.id ?? note.title}'),
        ...
        leading: Container(
          ...
          child: Icon(Icons.star_rounded, color: note.color, size: 22),
        ),
        title: Text(
          note.title,
